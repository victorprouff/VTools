...
public static class Sample4
{
    ...
    {
        Directory.CreateDirectory("sample4");
        using (var img = Image.Load("fb.jpg"))
        {
            ...
            using (var img2 = img.Clone(ctx =>
                       ctx.ApplyScalingWaterMark(font, "A short piece of text", Color.White, 5, false)))
            {
                img2.Save("sample4/simple.png");
            }


            using (var img2 = img.Clone(ctx => ctx.ApplyScalingWaterMark(font, LongText, Color.White, 5, true)))
            {
                img2.Save("sample4/wrapped.png");
            }
